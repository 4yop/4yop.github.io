...
`).filter(t=>t.trim().startsWith("`")).map(t=>{const s=t.trim().match(/`(\w+)`\s+(\w+(?:\([^)]+\))?)\s*(.*)/);return s?{name:s[1],type:s[2],comment:"",nullable:!s[3].includes("NOT NULL"),isPk:!1}:null}).filter(t=>t!==null)}async function _e(){var e;if(f.value.length!==0){G.value=!0;try{await new Promise(u=>setTimeout(u,600)),g.value=[];const t={laravel:ke,thinkphp:we,springboot:Pe,django:xe,express:je,gin:Ce},s=((e=t[B.value])==null?void 0:e.call(t))||[];g.value=s.filter(u=>{const n={model:"model",controller:"controller",service:"service",request:"request",migration:"migration",route:"route",factory:"factory",resource:"resource",vue:"vue",apiDoc:"api-doc",validate:"validate",entity:"entity",dto:"dto",vo:"vo",mapper:"mapper",mapperXml:"mapperXml",serviceImpl:"serviceImpl",serializer:"serializer",viewset:"viewset",url:"url",admin:"admin",test:"test",router:"router",middleware:"middleware",handler:"handler",response:"response"};return O.value.includes(n[u.filename]||u.filename)}),S.value=0}finally{G.value=!1}}}function ke(){const e=P.value,t=w.value,s=D(t),u=Z.value,n=f.value,o=L.value,l=[];return r.comments&&l.push({filename:"model",language:"php",filenameDisplay:`${e}.php`,content:`<?php
...
}`}),l}function xe(){var l;const e=P.value,t=w.value,s=f.value,u=L.value;e.charAt(0).toLowerCase()+e.slice(1);const n=[];let o=`from django.db import models
...
`)}
`}),n}function je(){const e=P.value,t=w.value;e.charAt(0).toLowerCase()+e.slice(1);const s=f.value,u=L.value,n=[];let o=s.map(l=>{const a=ce(l.type);let c=`    ${D(l.name)}: {
