...
`);j(s,`激活码列表_${H()}_${Date.now()}.txt`,"text/plain;charset=utf-8"),f("已导出 TXT 文件","ok")}function ke(){const s=u.value.map(a=>({code:a.code,expiresAt:a.expiresAt,expireText:a.expireText,generatedAt:Date.now(),config:{totalLength:b.value,groupSize:A.value,prefix:C.value,salt:d.value,expireDays:p.value}})),e=JSON.stringify(s,null,2);j(e,`激活码数据_${H()}_${Date.now()}.json`,"application/json;charset=utf-8"),f("已导出 JSON 文件","ok")}function H(){return window.location.hostname.replace(/\./g,"")}function j(s,e,a){const o=new Blob(["\uFEFF"+s],{type:a});X(o,e)}function X(s,e){const a=URL.createObjectURL(s),n=document.createElement("a");n.href=a,n.download=e,document.body.appendChild(n),n.click(),document.body.removeChild(n),URL.revokeObjectURL(a)}function Q(){return`/**
 * 激活码生成器 — 纯前端版本
 * 使用方式：
