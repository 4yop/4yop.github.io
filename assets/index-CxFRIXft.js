import{H as Le,m as i,o as Te,_ as Re,s as Ee,c,d as t,F as R,y as O,i as ee,v as te,l as m,t as r,h as Ve,n as K,f as ae,w as se,T as ne,P as je,Q as Ke,p as $,z as $e,a as l,j as Me,b as De}from"./index-CKZYrBL-.js";import{J as Pe}from"./jszip.min-CF03K1xG.js";import"./jszip.min-Cy7_Eeeu.js";import"./_commonjs-dynamic-modules-TDtrdbi3.js";const Ge={class:"lk-page"},He={class:"tab-bar"},Oe=["onClick"],Ne={class:"lk-body"},ze={class:"param-card"},Be={class:"core-params"},Ie={class:"field"},Ue=["value"],Je={class:"field"},Fe=["value"],Ze={class:"expire-row"},Ye={class:"chip-group"},Xe=["onClick"],Qe={class:"prefix-row"},We={class:"field flex-1"},qe=["value"],et={class:"field flex-1"},tt=["value"],at={class:"param-card action-card"},st={class:"batch-row"},nt={class:"field flex-1"},ot=["value"],rt=["disabled"],it={class:"result-section"},lt={key:0,class:"stats-bar"},ct={class:"stat-item"},dt={class:"stat-num"},ut={class:"stat-item"},pt={class:"stat-num"},vt={class:"stat-item"},ft={class:"stat-num"},ht=["disabled"],gt={class:"output-card"},mt={class:"output-header"},yt={key:0,class:"output-actions"},xt={class:"output-area"},bt={class:"line-num"},kt=["onClick"],wt={class:"line-expire"},Ct={key:0,class:"truncation-hint"},_t={key:1,class:"empty-hint"},St={class:"param-card action-card"},At={class:"verify-field"},Lt={class:"verify-action-row",style:{"margin-top":"0.85rem"}},Tt={key:0,class:"result-section"},Rt={class:"output-card"},Et={class:"verify-result-area"},Vt={class:"vr-icon"},jt={class:"vr-body"},Kt={class:"vr-status"},$t={class:"vr-reason"},Mt={key:0,class:"vr-meta"},Dt={key:0},Pt={class:"modal-box"},Gt={class:"modal-head"},v="ABCDEFGHJKLMNPQRSTUVWXYZ23456789",N=500,oe="lk_params_v1",Ht=Le({__name:"index",setup(Ot){const b=i(16),A=i(4),L=i(5),C=i(""),d=i(J()),T=i("30d"),p=i(30),u=i([]),M=i("复制全部"),D=i(!1),z=i(""),B=i("ok"),E=i(!1),_=i(!1),V=i("gen"),re=[{key:"gen",label:"生成激活码",icon:"🔑"},{key:"verify",label:"验证激活码",icon:"✅"}],P=i(""),ie=i(d.value),le=i(!1),h=i(null),ce=[{label:"1 天",value:"1d",days:1},{label:"7 天",value:"7d",days:7},{label:"30 天",value:"30d",days:30},{label:"90 天",value:"90d",days:90},{label:"365 天",value:"365d",days:365},{label:"永不过期",value:"never",days:-1},{label:"自定义",value:"custom",days:30}],I=$(()=>u.value.length>N),U=$(()=>I.value?u.value.slice(0,N):u.value),de=$(()=>`${b.value} 字符`),ue=$(()=>p.value===-1?"永久":p.value>=365&&p.value%365===0?`${p.value/365} 年`:`${p.value} 天`);function J(s=8){const e=new Uint8Array(s);return crypto.getRandomValues(e),Array.from(e,a=>v[a%v.length]).join("")}function f(s,e="ok"){z.value=s,B.value=e,D.value=!0,setTimeout(()=>{D.value=!1},2200)}function pe(){const s=C.value.toUpperCase().replace(/[^A-Z0-9]/g,""),e=b.value,a=Date.now(),n=p.value===-1?-1:a+p.value*864e5,o=new Uint8Array(e);crypto.getRandomValues(o);const S=Array.from(o,x=>v[x%v.length]).join(""),g=ve(S,n,d.value),y=`${g}:${d.value}`;let k=0;for(let x=0;x<y.length;x++)k=(k<<5)-k+y.charCodeAt(x)|0;const w=v[Math.abs(k)%v.length],H=g+w,Ae=s?`${s}${H}`:H;let q="永久有效";if(n!==-1){const x=new Date(n);q=`${x.getFullYear()}-${String(x.getMonth()+1).padStart(2,"0")}-${String(x.getDate()).padStart(2,"0")}`}return{code:Ae,raw:H,expiresAt:n,expireText:q}}function ve(s,e,a){const n=s.split(""),o=String(e),S=fe(a+o);for(let g=0;g<Math.min(o.length,n.length);g++){const y=(S+g*7)%n.length,k=(o.charCodeAt(g)+g*3)%v.length,w=v.indexOf(s[y]);w!==-1&&(n[y]=v[(w+k)%v.length])}return n.join("")}function fe(s){let e=0;for(let a=0;a<s.length;a++)e=(e<<5)-e+s.charCodeAt(a)|0;return Math.abs(e)}function F(s,e){const a=e||d.value,n=s.toUpperCase().trim();if(n.length<6)return{valid:!1,reason:"激活码长度不足",expired:!1,expiresAt:0};if(!/^[A-Z2-9]+$/.test(n))return{valid:!1,reason:"激活码包含非法字符",expired:!1,expiresAt:0};const o=n.slice(0,-1),S=n.slice(-1),g=o+":"+a;let y=0;for(let w=0;w<g.length;w++)y=(y<<5)-y+g.charCodeAt(w)|0;const k=v[Math.abs(y)%v.length];return S!==k?{valid:!1,reason:"激活码校验失败，可能被篡改",expired:!1,expiresAt:0}:{valid:!0,reason:"激活码格式正确",expired:!1,expiresAt:0}}function Z(){const s={totalLength:b.value,groupSize:A.value,batchCount:L.value,prefix:C.value,salt:d.value,expirePreset:T.value,expireDays:p.value};try{localStorage.setItem(oe,JSON.stringify(s))}catch{}}function he(){try{const s=localStorage.getItem(oe);if(!s)return!1;const e=JSON.parse(s);return e.totalLength!==void 0&&(b.value=e.totalLength),e.groupSize!==void 0&&(A.value=e.groupSize),e.batchCount!==void 0&&(L.value=e.batchCount),e.prefix!==void 0&&(C.value=e.prefix),e.salt?d.value=e.salt:d.value=J(),e.expirePreset!==void 0&&(T.value=e.expirePreset),e.expireDays!==void 0&&(p.value=e.expireDays),!0}catch{return!1}}async function ge(){he(),await $e(),await Y(),Z()}Te(async()=>{try{const{isPrerender:s}=await Re(async()=>{const{isPrerender:e}=await import("./test-Db6pn0qC.js");return{isPrerender:e}},[]);if(s())return}catch{}ge()}),Ee([b,A,L,C,d,T,p],()=>{Z()},{deep:!0});async function Y(){E.value=!0;const s=Math.max(1,Math.min(L.value,5e3));await new Promise(n=>setTimeout(n,10));const e=[],a=new Set;try{for(let n=0;n<s;n++){const o=pe();a.has(o.raw)?n--:(a.add(o.raw),e.push(o)),n>0&&n%500===0&&await new Promise(S=>setTimeout(S,0))}if(u.value=e,e.length>0){const n=F(e[0].code);n.valid||(console.error("[激活码自检失败]",e[0].code,n.reason),f("⚠️ 激活码自检未通过，请检查参数","err"))}f(`成功生成 ${e.length} 条激活码`,"ok")}catch{f("生成失败，请重试","err")}finally{E.value=!1}}async function me(){if(!u.value.length)return;const s=u.value.map(e=>e.code).join(`
`);try{await navigator.clipboard.writeText(s),M.value="已复制 ✓",f("已复制到剪贴板","ok"),setTimeout(()=>{M.value="复制全部"},2e3)}catch{f("复制失败，请手动选择复制","err")}}async function ye(s){try{await navigator.clipboard.writeText(s),f("已复制: "+s,"ok")}catch{}}function xe(){const s=P.value.trim();if(!s){f("请输入激活码","err");return}const e=ie.value||d.value,a=F(s,e),n=s.toUpperCase().trim();let o=null;/^[A-Z]{2,4}/.test(n)&&n.length>5&&(o=n.slice(0,n.match(/^[A-Z]+/)[0].length)),h.value={valid:a.valid,reason:a.reason,meta:{format:s,length:n.length,prefix:o}},f(a.valid?"激活码有效":"激活码无效",a.valid?"ok":"err")}function be(){const s=u.value.map(e=>`${e.code}	${e.expireText}`).join(`
`);j(s,`激活码列表_${G()}_${Date.now()}.txt`,"text/plain;charset=utf-8"),f("已导出 TXT 文件","ok")}function ke(){const s=u.value.map(a=>({code:a.code,expiresAt:a.expiresAt,expireText:a.expireText,generatedAt:Date.now(),config:{totalLength:b.value,groupSize:A.value,prefix:C.value,salt:d.value,expireDays:p.value}})),e=JSON.stringify(s,null,2);j(e,`激活码数据_${G()}_${Date.now()}.json`,"application/json;charset=utf-8"),f("已导出 JSON 文件","ok")}function G(){return window.location.hostname.replace(/\./g,"")}function j(s,e,a){const o=new Blob(["\uFEFF"+s],{type:a});X(o,e)}function X(s,e){const a=URL.createObjectURL(s),n=document.createElement("a");n.href=a,n.download=e,document.body.appendChild(n),n.click(),document.body.removeChild(n),URL.revokeObjectURL(a)}function Q(){return`/**
 * 激活码生成器 — 纯前端版本
 * 使用方式：
