...
const ClockApp = {
  ...
  cityTZ: { '北京': 8, '上海': 8, '东京': 9, '伦敦': 0, '巴黎': 1, '纽约': -5, '洛杉矶': -8, '悉尼': 10, '迪拜': 4, '新加坡': 8 },
  beep() {
    try {
  ...
  render(win) {
    ...
    win.timers.push(setInterval(() => {
      ...
      if (st.tab === 'sw' && st.sw.running) this._swTick(content, st);
      if (st.tab === 'timer' && st.timer.running) this._timerTick(content, st);
    }, 500));
    go('world');
  },
  render_world(box, st, save, soft) {
    ...
    list.innerHTML = '';
    st.data.cities.forEach(c => {
      const tz = this.cityTZ[c] ?? 8;
      const d = new Date(utc + tz * 3600000);
      const row = el('div', { class: 'clock-city' },
        el('div', null, el('b', { text: c }), el('small', { text: `UTC${tz >= 0 ? '+' : ''}${tz}` })),
        el('span', { class: 'clock-city-time', text: fmtTimeHM(d, Sys.settings.h24) }),
  ...
  render_alarm(box, st, save) {
    ...
    const renderList = () => {
      ...
      st.data.alarms.forEach(a => {
        const sw = el('div', { class: 'switch' + (a.on ? ' on' : '') });
        sw.addEventListener('click', () => { a.on = !a.on; sw.classList.toggle('on', a.on); save(); });
  ...
  render_sw(box, st) {
    box.innerHTML = '';
    const disp = el('div', { class: 'sw-disp', text: '00:00.0' });
    const startBtn = el('button', { class: 'btn primary sw-btn', text: '开始' });
    const lapBtn = el('button', { class: 'btn sw-btn', text: '计次', disabled: true });
    const resetBtn = el('button', { class: 'btn sw-btn', text: '复位', disabled: true });
    ...
    this._swTick = (c, s) => { disp.textContent = fmtT(cur()); };
    startBtn.addEventListener('click', () => {
      st.sw.running = !st.sw.running;
      if (st.sw.running) { st.sw.t0 = Date.now(); startBtn.textContent = '暂停'; lapBtn.disabled = false; resetBtn.disabled = true; }
      else { st.sw.acc += Date.now() - st.sw.t0; startBtn.textContent = '继续'; lapBtn.disabled = true; resetBtn.disabled = false; }
    });
    lapBtn.addEventListener('click', () => {
      st.sw.laps.push(cur());
      const i = st.sw.laps.length;
      laps.prepend(el('div', { class: 'sw-lap' }, el('span', { text: `计次 ${i}` }), el('span', { text: fmtT(st.sw.laps[i - 1]) })));
    });
    resetBtn.addEventListener('click', () => { st.sw = { running: false, t0: 0, acc: 0, laps: [] }; disp.textContent = '00:00.0'; laps.innerHTML = ''; startBtn.textContent = '开始'; resetBtn.disabled = true; });
  },
  render_timer(box, st) {
    box.innerHTML = '';
    ...
    const sync = () => { disp.textContent = fmtT(st.timer.left); ring.style.setProperty('--p', st.timer.total ? (1 - st.timer.left / st.timer.total) : 0); };
    this._timerTick = (c, s) => {
      if (!st.timer.running) return;
      st.timer.left = st.timer.endAt - Date.now();
      if (st.timer.left <= 0) {
        st.timer.running = false; st.timer.left = 0;
        Notify.send({ appId: 'clock', title: '计时器', body: '时间到！' });
        this.beep(); startBtn.textContent = '开始';
      }
      sync();
    };
    startBtn.addEventListener('click', () => {
      if (st.timer.running) { st.timer.running = false; startBtn.textContent = '继续'; return; }
      if (st.timer.left <= 0) {
      ...
      st.timer.running = true; startBtn.textContent = '暂停';
    });
    resetBtn.addEventListener('click', () => { st.timer = { left: 0, running: false, total: 0 }; startBtn.textContent = '开始'; sync(); disp.textContent = fmtT(((+mIn.value || 0) * 60 + (+sIn.value || 0)) * 1000); });
    ...
    get() {
      return Store.get('reminders', {
        ...
        items: [
          ...
          { id: uid(), list: 'l2', title: '周五前提交周报', due: new Date(Date.now() + 86400000 * 2).toISOString().slice(0, 10), done: false },
...
Apps.register({
  ...
  render(win) {
    ...
    win.onClose = () => { st.audio.pause(); st.audio.src = ''; };
...
Apps.register({
  ...
  render(win) {
    ...
    win.onClose = () => { if (video) { video.pause(); video.src = ''; } };
  }
});
