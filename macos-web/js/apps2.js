  ...
  render(win) {
    ...
    const bar = el('div', { class: 'music-bar' },
      ...
      el('div', { class: 'music-vol' }, (() => {
        const v = el('input', { type: 'range', class: 'slider', min: '0', max: '100', value: String(Math.round(Sys.settings.volume * 100)), title: '音量' });
        v.style.width = '76px';
        v.style.setProperty('--fill', (Sys.settings.volume * 100) + '%');
        v.addEventListener('input', () => { audio.volume = v.value / 100; v.style.setProperty('--fill', v.value + '%'); });
        return v;
      })()));
    ...
    win.onClose = () => { audio.pause(); audio.src = ''; Sys.mediaEls.delete(audio); };
...
Apps.register({
  ...
  render(win) {
    ...
    win.onClose = () => { video.pause(); video.src = ''; Sys.mediaEls.delete(video); };
    ...
    vol.addEventListener('input', () => { video.volume = vol.value / 100; vol.style.setProperty('--fill', vol.value + '%'); });
    playBtn.addEventListener('click', () => st.toggle());
...
Apps.register({
  ...
  render(win, args) {
    ...
    if (!st.path) {
      ...
      infoEl.textContent = '双击查看大图';
      return;
    }
    const asset = args.asset;
    const ext = ((asset || st.path).split('.').pop() || '').toLowerCase();
