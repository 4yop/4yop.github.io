...
const FinderApp = {
  ...
  render(win, args) {
    ...
    st.navigate = (p, push = true) => {
      ...
      if (push) { st.history = st.history.slice(0, st.hi + 1); st.history.push(st.path); st.hi++; }
      st.path = FS.normalize(p); st.selection.clear(); st.search = ''; searchInput.value = '';
      render();
    };
    ...
    const renderList = () => {
      ...
      for (const it of items) {
        ...
        itemEl.addEventListener('click', e => {
          e.stopPropagation();
          if (e.metaKey || e.ctrlKey) { st.selection.has(it.path) ? st.selection.delete(it.path) : st.selection.add(it.path); }
          else if (e.shiftKey && st.selection.size) {
            const arr = items.map(x => x.path); const a = arr.indexOf([...st.selection].pop()), b = arr.indexOf(it.path);
            st.selection = new Set(arr.slice(Math.min(a, b), Math.max(a, b) + 1));
          } else st.selection = new Set([it.path]);
          renderList();
          content.focus({ preventScroll: true });
        });
        ...
        itemEl.addEventListener('contextmenu', e => {
          ...
          if (!st.selection.has(it.path)) { st.selection = new Set([it.path]); renderList(); }
    ...
    content.addEventListener('click', () => { st.selection.clear(); renderList(); });
    ...
    content.addEventListener('keydown', e => {
      ...
      else if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'a') { e.preventDefault(); st.selection = new Set(FS.list(st.path).map(i => i.path)); renderList(); }
...
Apps.register({
  ...
  render(win) {
    ...
    const run = cmdline => {
      ...
      try {
        switch (cmd) {
          case 'help': print(['可用命令：', '  help  pwd  ls [路径]  cd <路径>  cat <文件>', '  touch <文件>  mkdir <目录>  rm <路径>  mv <源> <目标>  cp <源> <目标>', '  open <路径|应用>  echo <文本>  clear  date  whoami  uname', '路径支持 ~ 与相对路径，与访达共享同一文件系统。'].join('\n')); break;
          ...
          case 'cd': {
            const p = resolve(rest[0] || FS.HOME);
            if (!FS.isDir(p)) return err(`cd: 不是目录: ${rest[0] || ''}`);
            st.cwd = p; setPrompt(); break;
          }
          case 'cat': { if (!rest[0]) return err('cat: 缺少文件'); print(FS.read(resolve(rest[0]))); break; }
          case 'touch': { if (!rest[0]) return err('touch: 缺少文件'); const p = resolve(rest[0]); if (!FS.exists(p)) FS.write(p, ''); break; }
          case 'mkdir': { if (!rest[0]) return err('mkdir: 缺少目录'); FS.mkdir(resolve(rest[0])); break; }
          case 'rm': { if (!rest[0]) return err('rm: 缺少路径'); FS.remove(resolve(rest[0].replace(/^-[rf]$/, '') ? rest.find(a => !a.startsWith('-')) : rest[0])); break; }
          case 'mv': { if (rest.length < 2) return err('mv: 用法 mv <源> <目标>'); const dst = resolve(rest[1]); FS.move(resolve(rest[0]), FS.isDir(dst) ? dst : FS.dirName(dst) && FS.exists(dst) ? (err('mv: 目标已存在'), dst) : (() => { const d = FS.dirName(dst); const np = FS.move(resolve(rest[0]), d); return FS.rename(np, FS.baseName(dst)), dst; })()); break; }
          case 'cp': { if (rest.length < 2) return err('cp: 用法 cp <源> <目标>'); const s2 = resolve(rest[0]), d2 = resolve(rest[1]); FS.copy(s2, FS.isDir(d2) ? d2 : FS.dirName(d2)); break; }
          case 'open': {
            if (!rest[0]) return err('open: 缺少路径');
            const p = resolve(rest[0]);
            if (FS.exists(p)) Apps.openPath(p);
            else if (Apps.get(rest[0])) Apps.open(rest[0]);
            else err(`open: 找不到: ${rest[0]}`);
            break;
          }
...
Apps.register({
  ...
  render(win, args) {
    ...
    st.saveAs = async () => {
      ...
      try { FS.write(p, ta.value); st.path = p; st.dirty = false; refreshTitle(); }
      catch (e) { UI.alert('保存失败', e.message, this.icon); }
    };
    st.openPicker = async () => {
      const files = [];
    ...
    win.confirmClose = done => {
      if (!st.dirty) return done();
      UI.dialog({ icon: this.icon, title: '要存储更改吗？', msg: `“${name()}”有未存储的更改。`, buttons: ['不存储', '取消', '存储'] }).then(r => {
        if (r.index === 0) done();
        else if (r.index === 2) {
          if (st.path) { try { FS.write(st.path, ta.value); done(); } catch (e) { UI.alert('保存失败', e.message); } }
          else st.saveAs().then?.(() => { if (!st.dirty || st.path) done(); });
        }
      });
    };
...
Apps.register({
  ...
  render(win) {
    ...
    const st = win.appState = {
      ...
      sel: now.toISOString().slice(0, 10),
    ...
    st.goToday = () => { const t = new Date(); st.year = t.getFullYear(); st.month = t.getMonth(); st.sel = t.toISOString().slice(0, 10); render(); };
    const render = () => {
