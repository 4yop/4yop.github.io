...
const SettingsApp = {
  ...
  definePanes() {
    ...
    const toggle = (key, onChange) => {
      const sw = el('div', { class: 'switch' + (S[key] ? ' on' : ''), role: 'switch', 'aria-checked': String(!!S[key]), tabindex: '0' });
      const flip = () => { S[key] = !S[key]; sw.classList.toggle('on', S[key]); sw.setAttribute('aria-checked', String(S[key])); Sys.save(); Sys.applyAll(); onChange && onChange(S[key]); };
    ...
    P('wifi', 'Wi-Fi', '📶', '网络', box => {
      ...
      ['家庭网络 5G', 'CoffeeShop_Guest', 'Office-2.4G'].forEach((n, i) => {
        const btn = el('button', { class: 'btn', text: i === 0 && S.wifi ? '已连接' : '连接', disabled: !S.wifi || (i === 0 && S.wifi) });
        btn.addEventListener('click', () => { $$('.set-card .btn', list).forEach(b => { b.textContent = '连接'; b.disabled = false; }); btn.textContent = '已连接'; btn.disabled = true; });
        list.append(row(n, btn, i === 0 ? 'WPA2 个人级' : '开放'));
      });
      box.append(list, note(S.wifi ? '' : 'Wi-Fi 已关闭，打开后可加入网络。'));
    });
    P('bluetooth', '蓝牙', '🔵', '网络', box => {
      box.append(card(row('蓝牙', toggle('bluetooth'))));
      box.append(title('附近的设备'));
      const c = card();
      [['AirPods Pro', '耳机'], ['Magic Keyboard', '键盘'], ['MX Master 3S', '鼠标']].forEach(([n, t], i) => {
        const b = el('button', { class: 'btn', text: i === 0 ? '已连接' : '连接', disabled: !S.bluetooth || i === 0 });
        b.addEventListener('click', () => { b.textContent = '已连接'; b.disabled = true; });
        c.append(row(n, b, t));
      });
      box.append(c);
    });
    ...
    P('notifications', '通知', '🔔', '系统', box => {
      box.append(card(row('允许通知', toggle('_notifGlobal', on => { S._notifGlobal = on; }), '关闭后不再显示横幅')));
      box.append(title('应用通知'));
      const c = card();
      ['mail', 'messages', 'calendar', 'reminders', 'weather', 'appstore'].forEach(id => {
        const app = Apps.get(id); if (!app) return;
        const key = 'notif_' + id;
        if (!(key in S)) S[key] = true;
        c.append(row(app.name, toggle(key), '横幅与通知中心'));
      });
      box.append(c);
    });
    P('sound', '声音', '🔊', '系统', box => {
    ...
    P('datetime', '日期与时间', '🕐', '通用', box => {
      box.append(card(
        ...
        row('时区', (() => {
          const s = el('select', { class: 'text-input' }, ...['亚洲/上海 (UTC+8)', '亚洲/东京 (UTC+9)', '欧洲/伦敦 (UTC+0)', '美洲/纽约 (UTC-5)'].map(z => el('option', { text: z })));
          return s;
        })())));
    ...
    P('loginitems', '登录项', '🔑', '通用', box => {
      ...
      ['mail', 'messages', 'notes', 'calendar', 'music'].forEach(id => {
        const app = Apps.get(id); if (!app) return;
        const sw = el('div', { class: 'switch' + (S.loginItems[id] ? ' on' : '') });
        sw.addEventListener('click', () => { S.loginItems[id] = !S.loginItems[id]; sw.classList.toggle('on', S.loginItems[id]); Sys.save(); });
        c.append(row(app.name, sw));
      });
    ...
    P('controlcenter', '控制中心', '🎛', '系统外观', box => {
      ...
      box.append(note('通过菜单栏右上角图标随时打开控制中心。'));
    });
    ...
    P('privacy', '隐私与安全性', '🔒', '系统外观', box => {
      box.append(card(row('防火墙', toggle('_firewall'), '阻止未授权的传入连接')));
    ...
    P('dock', '桌面与程序坞', '🖥', '系统外观', box => {
      ...
      const magRow = row('放大', (() => {
        ...
        const sw = el('div', { class: 'switch' + (S.dockMagnify ? ' on' : '') });
        ...
        sync();
        sw.addEventListener('click', () => { S.dockMagnify = !S.dockMagnify; sw.classList.toggle('on', S.dockMagnify); Sys.save(); });
      ...
      box.append(card(row('自动隐藏和显示程序坞', (() => {
        const sw = el('div', { class: 'switch' + (S.dockAutohide ? ' on' : '') });
        sw.addEventListener('click', () => { S.dockAutohide = !S.dockAutohide; sw.classList.toggle('on', S.dockAutohide); Sys.save(); Sys.layoutDock(); });
        return sw;
      })())));
    });
    ...
    P('lockscreen', '锁定屏幕', '🔐', '安全', box => {
      const sw = el('div', { class: 'switch' + (S.passwordEnabled ? ' on' : '') });
      sw.addEventListener('click', async () => {
        ...
        sw.classList.toggle('on', S.passwordEnabled); Sys.save();
    ...
    P('keyboard', '键盘', '⌨️', '输入', box => {
      box.append(card(
        row('按键重复速度', (() => { const r = el('input', { type: 'range', class: 'slider', min: '1', max: '10', value: '7' }); r.style.setProperty('--fill', '67%'); r.addEventListener('input', () => r.style.setProperty('--fill', (r.value - 1) / 9 * 100 + '%')); return r; })()),
        row('重复前延迟', (() => { const r = el('input', { type: 'range', class: 'slider', min: '1', max: '10', value: '4' }); r.style.setProperty('--fill', '33%'); r.addEventListener('input', () => r.style.setProperty('--fill', (r.value - 1) / 9 * 100 + '%')); return r; })())));
      box.append(card(row('输入法', el('span', { text: '简体拼音' })), row('键盘亮度', el('span', { text: '自动' }))));
    });
    P('mouse', '鼠标', '🖱', '输入', box => {
      box.append(card(
        row('跟踪速度', (() => { const r = el('input', { type: 'range', class: 'slider', min: '1', max: '10', value: '5' }); r.style.setProperty('--fill', '44%'); r.addEventListener('input', () => r.style.setProperty('--fill', (r.value - 1) / 9 * 100 + '%')); return r; })()),
